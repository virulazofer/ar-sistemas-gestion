<x-app-layout>
        ...
        <div class="ar-card overflow-x-auto">
            <table class="ar-table text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="14" class="ar-muted py-6 text-center">No hay movimientos pendientes.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    @can('categories.edit')
        <div class="grid gap-4 lg:grid-cols-2">
            <form method="POST" action="{{ route('chart-accounts.normalize-super') }}" class="ar-card space-y-2 p-4">
                @csrf
                <h2 class="font-semibold">Normalizar Super → Supermercado</h2>
                <p class="ar-muted text-sm">Conserva trazabilidad histórica (excel / alias). Siempre con preview.</p>
                <button name="apply" value="0" class="ar-btn ar-btn-secondary">Vista previa</button>
                <button name="apply" value="1" class="ar-btn ar-btn-primary" onclick="return confirm('¿Aplicar Super→Supermercado?')">Aplicar</button>
                @if (session('super_preview'))
                    <pre class="overflow-x-auto rounded border p-2 text-xs" style="border-color: var(--ar-border);">{{ json_encode(session('super_preview'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                @endif
            </form>
            <form method="POST" action="{{ route('chart-accounts.professional-incomes') }}" class="ar-card space-y-2 p-4">
                @csrf
                <h2 class="font-semibold">Ingresos profesionales confirmados</h2>
                <p class="ar-muted text-sm">Abonos / Reparaciones / Instalaciones / Remotos (+ Ventas como clasificación económica, sin duplicar circuito).</p>
                <button name="apply" value="0" class="ar-btn ar-btn-secondary">Vista previa</button>
                <button name="apply" value="1" class="ar-btn ar-btn-primary">Aplicar mapeos</button>
                @if (session('professional_income_report'))
                    <pre class="overflow-x-auto rounded border p-2 text-xs" style="border-color: var(--ar-border);">{{ json_encode(session('professional_income_report'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                @endif
            </form>
        </div>
        <p class="mt-3 text-sm"><a href="{{ route('chart-accounts.semantics') }}" class="underline" style="color: var(--ar-brand);">Análisis Comida / Auto / Miranda / MYU (sin aplicar)</a></p>
    @endcan
</x-app-layout>
