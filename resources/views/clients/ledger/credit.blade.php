<x-app-layout>
    <x-slot name="header"><h1 class="text-xl font-semibold">Crédito a favor — {{ $client->name }}</h1></x-slot>
    <form method="POST" action="{{ route('clients.ledger.credit.store', $client) }}" class="ar-card mx-auto max-w-lg space-y-4 p-6">
        @csrf
        <p class="ar-muted text-sm">Registra saldo a favor del cliente. No mueve caja/banco.</p>
        <div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button class="ar-btn ar-btn-primary">Registrar crédito</button>
        </div>
    </form>
</x-app-layout>
